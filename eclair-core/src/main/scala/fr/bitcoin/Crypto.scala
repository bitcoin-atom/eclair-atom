...
object Crypto {
  ...
  def encodeSignatureBCA(t: (BigInteger, BigInteger)): BinaryData =
    encodeSignature(t._1, t._2) :+ fr.acinq.bitcoin.SIGHASH_BCAFORK.toByte

  def encodeSignatureBCA(r: BigInteger, s: BigInteger): Seq[Byte] =
    encodeSignature(r, s) :+ fr.acinq.bitcoin.SIGHASH_BCAFORK.toByte
  ...
  def checkSignatureEncoding(sig: Seq[Byte], flags: Int): Boolean = {
    ...
    if (sig.isEmpty) true
      ...
      println("SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_STRICTENC")
      false
    }
    else if ((flags & SCRIPT_VERIFY_LOW_S) != 0 && !isLowDERSignature(sig)) {
      println("SCRIPT_VERIFY_LOW_S")
      false
    }
    else if ((flags & SCRIPT_VERIFY_STRICTENC) != 0 && !isDefinedHashtypeSignature(sig)) {
      println("SCRIPT_VERIFY_STRICTENC")
      false
    }
    else true
  }
  ...
  def isDefinedHashtypeSignature(sig: Seq[Byte]): Boolean = if (sig.isEmpty) false
    ...
    val hashType = (sig.last & 0xff) & ~(SIGHASH_ANYONECANPAY | FORK_ID)
