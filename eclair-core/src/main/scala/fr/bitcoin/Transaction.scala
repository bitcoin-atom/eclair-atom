...
object Transaction extends BtcSerializer[Transaction] {
  ...
  /**
    * Sign a transaction. Cannot partially sign. All the input are signed with SIGHASH_BCAFORK
    *
  ...
  def sign(input: Transaction, signData: Seq[SignData]): Transaction = {
    ...
    val signedInputs = for (i <- input.txIn.indices) yield {
      val sig = signInput(input, i, signData(i).prevPubKeyScript, SIGHASH_BCAFORK, 0 satoshi, SigVersion.SIGVERSION_BASE, signData(i).privateKey)
