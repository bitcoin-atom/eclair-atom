...
package object eclair {
  ...
  def addressToPublicKeyScript(address: String, chainHash: BinaryData): Seq[ScriptElt] = {
    Try(Base58Check.decode(address)) match {
      case Success((Base58.Prefix.PubkeyAddressTestnet, pubKeyHash)) if chainHash == Block.BCATestnetForkBlockHash || chainHash == Block.BCARegtestForkBlockHash => Script.pay2pkh(pubKeyHash)
      case Success((Base58.Prefix.PubkeyAddress, pubKeyHash)) if chainHash == Block.BCALivenetForkBlockHash => Script.pay2pkh(pubKeyHash)
      case Success((Base58.Prefix.ScriptAddressTestnet, scriptHash)) if chainHash == Block.BCATestnetForkBlockHash || chainHash == Block.BCARegtestForkBlockHash => OP_HASH160 :: OP_PUSHDATA(scriptHash) :: OP_EQUAL :: Nil
      case Success((Base58.Prefix.ScriptAddress, scriptHash)) if chainHash == Block.BCALivenetForkBlockHash => OP_HASH160 :: OP_PUSHDATA(scriptHash) :: OP_EQUAL :: Nil
      ...
      case Failure(base58error) =>
        Try(Bech32.decodeWitnessAddress(address)) match {
          ...
          case Success(("bca", _, bin)) if chainHash == Block.BCALivenetForkBlockHash => OP_0 :: OP_PUSHDATA(bin) :: Nil
          case Success(("tbca", _, bin)) if chainHash == Block.BCATestnetForkBlockHash => OP_0 :: OP_PUSHDATA(bin) :: Nil
          case Success(("bcart", _, bin)) if chainHash == Block.BCARegtestForkBlockHash => OP_0 :: OP_PUSHDATA(bin) :: Nil
