...
object HtlcGenerationSpec {
  ...
  val sig = Crypto.encodeSignatureBCA(Crypto.sign(Crypto.sha256(BinaryData.empty), priv_a.privateKey))
  val defaultChannelUpdate = ChannelUpdate(sig, Block.BCARegtestForkBlockHash, ShortChannelId(0), 0, 1, 0, 0, 42000, 0, 0, Some(500000000L))
