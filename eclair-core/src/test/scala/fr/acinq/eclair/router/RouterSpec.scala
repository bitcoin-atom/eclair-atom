...
class RouterSpec extends BaseRouterSpec {
  ...
  test("properly announce valid new channels and ignore invalid ones") { fixture =>
    ...
    val update_ac = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, c, channelId_ac, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, 500000000L)
    ...
    val update_ax = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, priv_x.publicKey, chan_ax.shortChannelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, 500000000L)
    ...
    val update_ay = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, priv_y.publicKey, chan_ay.shortChannelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, 500000000L)
    // a-z will be spent
    ...
    val update_az = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, priv_z.publicKey, chan_az.shortChannelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, 500000000L)
  ...
  test("route not found (channel disabled)") { fixture =>
    ...
    assert(res.hops.map(_.nodeId).toList === a :: b :: c :: Nil)
    assert(res.hops.last.nextNodeId === d)
    val channelUpdate_cd1 = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_c, d, channelId_cd, cltvExpiryDelta = 3, 0, feeBaseMsat = 153000, feeProportionalMillionths = 4, htlcMaximumMsat = 500000000L, enable = false)
  ...
  test("ask for channels that we marked as stale for which we receive a new update") { fixture =>
    ...
    val update = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, c, channelId, cltvExpiryDelta = 7, htlcMinimumMsat = 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, htlcMaximumMsat = 5, timestamp = timestamp)
    val probe = TestProbe()
    ...
    val update1 = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, c, channelId, cltvExpiryDelta = 7, htlcMinimumMsat = 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, htlcMaximumMsat = 500000000L, timestamp = Platform.currentTime / 1000)
