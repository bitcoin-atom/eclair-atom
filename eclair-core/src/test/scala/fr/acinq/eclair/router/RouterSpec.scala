...
class RouterSpec extends BaseRouterSpec {
  ...
  test("properly announce valid new channels and ignore invalid ones") { case (router, watcher) =>
    ...
    val update_ac = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, c, channelId_ac, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10)
    ...
    val update_ax = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, priv_x.publicKey, chan_ax.shortChannelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10)
    ...
    val update_ay = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, priv_y.publicKey, chan_ay.shortChannelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10)
    // a-z will be spent
    ...
    val update_az = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, priv_z.publicKey, chan_az.shortChannelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10)
  ...
  test("route not found (channel disabled)") { case (router, _) =>
    ...
    val channelUpdate_cd1 = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_c, d, channelId_cd, cltvExpiryDelta = 3, 0, feeBaseMsat = 153000, feeProportionalMillionths = 4, enable = false)
  ...
  test("ask for channels that we marked as stale for which we receive a new update") { case (router, watcher) =>
    ...
    val update = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, c, channelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, timestamp = timestamp)
    val probe = TestProbe()
    ...
    val update1 = makeChannelUpdate(Block.BCARegtestForkBlockHash, priv_a, c, channelId, cltvExpiryDelta = 7, 0, feeBaseMsat = 766000, feeProportionalMillionths = 10, timestamp = Platform.currentTime / 1000)
