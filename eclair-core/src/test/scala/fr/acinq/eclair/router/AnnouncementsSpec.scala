...
class AnnouncementsSpec extends FunSuite {
  ...
  test("create valid signed channel announcement") {
    ...
    val (node_a_sig, bitcoin_a_sig) = signChannelAnnouncement(Block.BCARegtestForkBlockHash, ShortChannelId(42L), node_a, node_b.publicKey, bitcoin_a, bitcoin_b.publicKey, "")
    val (node_b_sig, bitcoin_b_sig) = signChannelAnnouncement(Block.BCARegtestForkBlockHash, ShortChannelId(42L), node_b, node_a.publicKey, bitcoin_b, bitcoin_a.publicKey, "")
    val ann = makeChannelAnnouncement(Block.BCARegtestForkBlockHash, ShortChannelId(42L), node_a.publicKey, node_b.publicKey, bitcoin_a.publicKey, bitcoin_b.publicKey, node_a_sig, node_b_sig, bitcoin_a_sig, bitcoin_b_sig)
  ...
  test("create valid signed channel update announcement") {
    val ann = makeChannelUpdate(Block.BCARegtestForkBlockHash, Alice.nodeParams.privateKey, randomKey.publicKey, ShortChannelId(45561L), Alice.nodeParams.expiryDeltaBlocks, Alice.nodeParams.htlcMinimumMsat, Alice.nodeParams.feeBaseMsat, Alice.nodeParams.feeProportionalMillionth, 500000000L)
  ...
  test("check flags") {
    ...
    val channelUpdate1 = makeChannelUpdate(Block.BCARegtestForkBlockHash, node1_priv, node2_priv.publicKey, ShortChannelId(0), 0, 0, 0, 0, 500000000L, enable = true)
    val channelUpdate1_disabled = makeChannelUpdate(Block.BCARegtestForkBlockHash, node1_priv, node2_priv.publicKey, ShortChannelId(0), 0, 0, 0, 0, 500000000L, enable = false)
    val channelUpdate2 = makeChannelUpdate(Block.BCARegtestForkBlockHash, node2_priv, node1_priv.publicKey, ShortChannelId(0), 0, 0, 0, 0, 500000000L, enable = true)
    val channelUpdate2_disabled = makeChannelUpdate(Block.BCARegtestForkBlockHash, node2_priv, node1_priv.publicKey, ShortChannelId(0), 0, 0, 0, 0, 500000000L, enable = false)
