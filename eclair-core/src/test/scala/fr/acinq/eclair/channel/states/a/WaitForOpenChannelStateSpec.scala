...
class WaitForOpenChannelStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("recv OpenChannel (funding too low)") { case (bob, alice2bob, bob2alice, _) =>
    within(30 seconds) {
      ...
      val error = bob2alice.expectMsgType[Error]
      assert(error === Error(open.temporaryChannelId, new InvalidFundingAmount(open.temporaryChannelId, lowFundingMsat, Bob.nodeParams.minFundingSatoshis).getMessage.getBytes("UTF-8")))
      awaitCond(bob.stateName == CLOSED)
    }
  }
