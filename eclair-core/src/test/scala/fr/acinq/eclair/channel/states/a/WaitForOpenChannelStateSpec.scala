...
class WaitForOpenChannelStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("recv OpenChannel (invalid chain)") { f =>
    ...
    assert(error === Error(open.temporaryChannelId, InvalidChainHash(open.temporaryChannelId, Block.BCARegtestForkBlockHash, livenetChainHash).getMessage.getBytes("UTF-8")))
  ...
  test("recv OpenChannel (funding too low)") { f =>
    ...
    val error = bob2alice.expectMsgType[Error]
    assert(error === Error(open.temporaryChannelId, InvalidFundingAmount(open.temporaryChannelId, lowFundingMsat, Bob.nodeParams.minFundingSatoshis).getMessage.getBytes("UTF-8")))
    awaitCond(bob.stateName == CLOSED)
  }
