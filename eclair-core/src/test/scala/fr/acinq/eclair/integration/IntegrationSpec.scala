...
class IntegrationSpec extends TestKit(ActorSystem("test")) with BitcoindService with FunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("wait atomd ready") {
    val sender = TestProbe()
    logger.info(s"waiting for atomd to initialize...")
    awaitCond({
  ...
  test("send an HTLC A->D with an invalid expiry delta for B") {
    ...
    val channelUpdateBC = Announcements.makeChannelUpdate(Block.BCARegtestForkBlockHash, nodes("B").nodeParams.privateKey, nodes("C").nodeParams.nodeId, shortIdBC, nodes("B").nodeParams.expiryDeltaBlocks + 1, nodes("C").nodeParams.htlcMinimumMsat, nodes("B").nodeParams.feeBaseMsat, nodes("B").nodeParams.feeProportionalMillionth, 500000000L)
